...
func fire_weapon():
    var area = $Area
    var bodies = area.get_overlapping_bodies() #otteniamo il nodo figlio Area
 #elenco di corpi che toccano l'area
    for body in bodies:
        if body == player_node: #controlliamo che il giocatore non si pugnali da solo
            continue  
